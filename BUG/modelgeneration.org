...
#+NAME: DiscreteMC1Parameters
#+begin_src python  :session *pyshard4* :var paths_parameter=200 :noweb yes
...
#+begin_src python  :session *pyshard4* :var parameter=200 :noweb yes 
  <<ImportsFeniaxShard>>
  np.random.seed(2025)
  sol = "cao"
  paths = 8*paths_parameter
  num_modes = 100
  device_count = 1
  <<MainBug>>
  inp.simulation.typeof = "single"
...
#+begin_src python :tangle settings_ADDiscreteMC1_t.py :session *pyshard4* :noweb yes 
  <<DiscreteMC1high()>>
  <<parameters_gafs(output="label_flow")>>
  inp.driver.sol_path = pathlib.Path(
...
#+begin_src python :tangle settings_gust1shard.py :session *pyshard2*
  ...
  inp.driver.sol_path = pathlib.Path(
      f"{results_path}/gust3_{sol}Shard")
  inp.system.aero.gust.fixed_discretisation = [150, u_inf]
  # Shard inputs
